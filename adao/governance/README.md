# aDAO / governance — CURATED (migrated 2026-08-11 from defipatriot/adao_json_storage)
...
|---|---|---|
| `registry.json` | vetted contract address book: per-address `name`/`type`/`validActions` + `lpPools` map. 44 contracts. | hand-curated |
| `proposals.json` | 37 proposals with votes/turnout/quorum/threshold | exported 2026-05-22 — **STALE**, re-derive from chain (queued) |
| `members.csv` | 157 members: address, name, image, staked, VP | snapshot export |
